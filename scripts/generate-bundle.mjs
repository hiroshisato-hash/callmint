      ...
      tweets: {
        ...
        items: {
          ...
          properties: {
            ...
            mode: {
              type: "string",
              enum: ["ask", "devlog", "idea", "social"],
              description: "ask=教えて系の質問 / devlog=開発実況 / idea=機能アイデア相談 / social=ゆるい交流・フォローDM誘導",
            },
            topic_label: { type: "string" },
          },
...
async function generateThursdayThread(weekStartDate) {
  ...
  const result = await callClaude({
    model: MODEL_SONNET,
    userPrompt: `木曜日に投稿する、Callmint公式マスコット「ぴよ」の独立Xスレッドを作成してください。今週のnote記事とは別トピックです。

【トピック】
${topic.title}（${topic.angle}）

【スレッド要件】
- 4〜5ツイート、全文「ぴよ」口調（一人称「ぴよ」、語尾「〜ぴよ」）
- 立ち位置: 美容は素人なので教わる側。電話AI・開発の話は等身大に具体的に（誇張・盛り禁止）
- 1本目: 強フック（開発実況 / 現場への問いかけ / 意外な気づき）+「以下スレ🧵」
- 中間: 単体でも読める内容。開発で分かったこと or 現場に聞きたいこと
- 最終本: 「気軽にリプ・DMで教えてほしいぴよ」などフォロー・交流の誘導
- note誘導はしない（単発スレ）
- 各140字以内厳守
...
async function generateTweetPool() {
  ...
  const result = await callClaude({
    model: MODEL_SONNET,
    userPrompt: `美容師・サロンオーナー向けの、Callmint公式マスコット「ぴよ」のX単発ツイートを **14本** 生成してください。

${SINGLE_TWEET_VOICE}

【内訳（4本柱に振り分け）】
- ① 教えて系: 4本（電話・予約・無断キャンセルなど現場の困りごとを質問。Yes/Noか二択で答えやすく）
- ② 開発実況: 4本（Callmintで今やってること・改良・つまずき・解決を実況。具体的に）
- ③ アイデア相談: 3本（「こういう機能どう？」と案を投げて現場の反応を聞く）
- ④ ゆるい交流: 3本（素人だけど本気で作ってる姿勢＋フォロー・リプ・DMを気軽に誘う）

【重要な姿勢】
- 美容ノウハウを上から語らない・専門家ぶらない。あくまで「教わる側」「一緒に作る側」
- 開発実況は「作ってる感」が伝わる具体的な描写を入れる（誇張・盛った数字は禁止）
...
【重要ルール】
- 各ツイートは独立で読めること
- 14本通して話題を散らすこと（同じ切り口2本以上禁止）
- ブランドガイドのNG表現禁止。全ツイート「ぴよ」口調で統一
- 各140字以内厳守（ハッシュタグ込みで140字）
